...
An autoresearch-style experiment loop to optimize a markdown-to-HTML converter.
...
3. Create `results.tsv` with the header row if it doesn't exist.
...
- Look at `test_data.py` expected HTML and hardcode the answers. You must write a general-purpose converter.
...
**Hints:** Study the evaluate.py scoring to understand what matters. The evaluator compares HTML DOM trees node-by-node: tag names, attributes, text content, and children. Fix one feature at a time — start with headings, then inline formatting, then block elements. Each improvement should be a small, testable change.
